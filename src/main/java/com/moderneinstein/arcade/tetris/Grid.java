...
//current = spawner.nextBrick() ;
        ...
        current.velocity = blockVelocity ;
		current.update(G4)  ;
		boolean bool1 = checkProgress()  ;
		if(bool1==true){
			current =  spawner.list4.get(spanwer.counter)  ;
		}
		checkFilled() ;
		this.paint(G4) ;
	}
    public synchronized void shuffle(int leap){
        int size1 = spawner.list4.size() ; 
        int size2  = spawner.counter+leap ;
         TetrisBlock block4 = spawner.list4.get(size2%size1) ;
         if(block4!=null){
            current = block4 ;
            spanwer.counter  = spawner.counter+1 ;
         }
         spawner.run() ;
    }
